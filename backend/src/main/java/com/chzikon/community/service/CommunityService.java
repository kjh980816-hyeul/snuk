...
/**
 * 스눅 커뮤니티.
 * 게시판 = 어드민이 코드 없이 관리(트리 2단), 글/댓글 = 로그인 회원,
...
public class CommunityService {
    ...
    /** 공개 목록 — 노출 중인 게시판만(하위는 상위가 숨김이면 함께 숨김). */
    @Transactional(readOnly = true)
    public List<BoardResponse> boards() {
        List<CommunityBoard> all = boardRepository.findAllByOrderBySortOrderAscIdAsc();
        Set<Long> hiddenGroups = all.stream()
                .filter(b -> b.isGroup() && !b.isVisible())
                .map(CommunityBoard::getId)
                .collect(Collectors.toSet());
        return all.stream()
                .filter(CommunityBoard::isVisible)
                .filter(b -> b.isGroup() || !hiddenGroups.contains(b.getParentId()))
    ...
    public List<BoardResponse> allBoards() {
        return boardRepository.findAllByOrderBySortOrderAscIdAsc().stream().map(BoardResponse::of).toList();
    }
    ...
    public BoardResponse updateBoard(Long id, BoardRequest req) {
        ...
        board.update(req.name(), req.sortOrder(), req.visible(), null, null);
        return BoardResponse.of(board);
    }

    /** 삭제 — 글이나 하위 게시판이 있으면 거절(숨김으로 유도). */
    @Transactional
    ...
    /**
     * 목록. boardId=null 이면 전체.
     * sort: new(최신) / hot(조회) / buff(버프)
     */
    @Transactional(readOnly = true)
    public PostPage list(Long boardId, String sort, String q, int page, int size) {
        int safeSize = Math.min(Math.max(size, 1), MAX_PAGE_SIZE);
        Pageable pageable = PageRequest.of(Math.max(page, 0), safeSize, sortOf(sort));
        String pattern = patternOf(q);

        Page<CommunityPost> found;
        if (boardId == null) {
            found = postRepository.search(pattern, pageable);
        } else {
            found = postRepository.searchInBoards(boardScope(boardId), pattern, pageable);
        }
        return new PostPage(summaries(found.getContent()), found.getNumber(), found.getSize(),
                found.getTotalElements(), found.getTotalPages());
    }

    /** 인기글(사이드바) — 게시판 지정 시 그 범위 안에서. */
    @Transactional(readOnly = true)
    public List<PostSummary> popular(Long boardId, int limit) {
        Collection<Long> scope = boardId == null ? List.of(0L) : boardScope(boardId);
        List<CommunityPost> posts = postRepository.findPopular(boardId == null, scope,
    ...
    public PostDetail write(Long memberId, PostRequest req) {
        CommunityBoard board = writableBoard(req.boardId());
    ...
    public PostDetail edit(Long id, Long memberId, PostRequest req) {
        ...
        Long boardId = req.boardId() != null ? writableBoard(req.boardId()).getId() : null;
    ...
    /** 글을 쓸 수 있는 게시판 = 노출 중 + (하위 게시판이 없는 그룹 or 하위 게시판). */
    private CommunityBoard writableBoard(Long boardId) {
        CommunityBoard board = getBoard(boardId);
        if (!board.isVisible()) {
            throw new BusinessException(ErrorCode.BOARD_NOT_AVAILABLE);
        }
        if (board.isGroup() && boardRepository.existsByParentId(board.getId())) {
            throw new BusinessException(ErrorCode.BOARD_NOT_AVAILABLE);
        }
        if (!board.isGroup()) {
            CommunityBoard parent = getBoard(board.getParentId());
            if (!parent.isVisible()) {
                throw new BusinessException(ErrorCode.BOARD_NOT_AVAILABLE);
            }
        }
    ...
    private PostDetail detailOf(CommunityPost post, Long memberId) {
        ...
        CommunityBoard group = board != null && board.getParentId() != null
                ? boardRepository.findById(board.getParentId()).orElse(null) : null;
